...
constexpr auto
make_error_category( std::string_view name, ErrorCode success_code, Messages&& messages ) {
...
class archive
{
  public:
    ...
    auto operator()( Items&&... items ) {
        ...
        static_assert(
            detail::all_of<!std::is_pointer<std::remove_reference_t<Items>>::value...>::value,
  ...
  private:
    ...
    template <typename Item,
              typename...,
              typename =
                  decltype( std::remove_reference_t<Item>::serialize( std::declval<archive_type&>(),
                                                                      std::declval<Item&>() ) )>
    ...
    auto serialize_item( Item&& item ) {
        ...
        using integral_type =
            std::conditional_t<std::is_const<std::remove_reference_t<Item>>::value,
...
class memory_input_archive : private memory_view_input_archive
{
  public:
    ...
    auto operator()( Items&&... items ) {
        ...
        // Load the items.
        if ( auto result = memory_view_input_archive::operator()( std::forward<Items>( items )... );
             !result ) {
            // Reset the offset back.
            reset( offset );
            return result;
...
auto serialize( Archive& archive, Container& container ) {
    ...
    return archive(
        as_bytes( std::addressof( container[0] ), static_cast<SizeType>( container.size() ) ) );
}
    ...
    for ( std::size_t i {}; i < ( aligned_message_size / sizeof( std::uint32_t ) ); i += 16 ) {
        ...
        for ( std::size_t j = 16; j < 80; ++j ) {
            w[j] = detail::swap_byte_order( detail::rotate_left(
                detail::swap_byte_order( w[j - 3] ^ w[j - 8] ^ w[j - 14] ^ w[j - 16] ), 1 ) );
        }
