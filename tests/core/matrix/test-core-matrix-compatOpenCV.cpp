...
//            assert( eigenMat.coeff( i, j ) == cvMat.data[i * height + j] );
    ...
    //    hub::format::DISPARITY16, 1  } ); std::cout << "resolutions: " <<
    //    resolutions << std::endl;

    //    hub::sensor::Resolution resolution { hub::format::BGR8, 1 };
    //    CHECK(resolution.nByte() == 3);

    //    hub::sensor::Resolution resolution2 { hub::format::Y8, 6 };
    ...
    ////    static_assert(hub::format::RGB8. == "RGB8");

    ////    auto & format = hub::format::RGB8;

    ////    hub::sensor::Format format = hub::format::NONE;
