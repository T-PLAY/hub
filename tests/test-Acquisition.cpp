...
TEST_CASE( "Acquisition test" ) {
    ...
    // getSize
    {
        ...
        acq << hub::Measure { data, 3, hub::sensor::Resolution { { 1 }, hub::sensor::Format::BGR8 } };
        CHECK( acq.getSize() == 3 );

        acq.emplaceMeasure( data, 3, hub::sensor::Resolution { { 1 }, hub::sensor::Format::BGR8 } );
        CHECK( acq.getSize() == 6 );

        acq.pushBack(
            hub::Measure { data, 3, hub::sensor::Resolution { { 1 }, hub::sensor::Format::BGR8 } } );
        CHECK( acq.getSize() == 9 );
    }
    ...
    hub::sensor::Resolution resolution( { 1 }, hub::sensor::Format::RGB8 );
    ...
    for ( int i = 1; i < (int)hub::sensor::Format::COUNT; ++i ) {
        ...
        hub::sensor::Acquisition acq22 =
            std::move( hub::sensor::Acquisition( 0, 0 ) << hub::Measure(
                           data, hub::sensor::resolution::computeAcquisitionSize( resolution2 ), resolution2 ) );
        CHECK( acq22.isInterpolable() == hub::sensor::resolution::s_format2isInterpolable[(int)format] );
    }
    ...
    CHECK( acq45 == ( hub::sensor::Acquisition( 1, 1 ) << hub::data::Dof6( 0.5, 0.5, 0.5, 1, 0, 0, 0 ) ) );
    ...
    // hasFixedSize
    {
        ...
        acq8 << hub::Measure { data3, 3, hub::sensor::Resolution { { 1 }, hub::sensor::Format::BGR8 } };
        CHECK( acq8.hasFixedSize() );

        acq8.emplaceMeasure( data3, 3, hub::sensor::Resolution { { 1 }, hub::sensor::Format::BGR8 } );
        CHECK( acq8.hasFixedSize() );

        acq8.pushBack(
            hub::Measure { data3, 3, hub::sensor::Resolution { { 1 }, hub::sensor::Format::USER_DATA } } );
        CHECK( !acq8.hasFixedSize() );
    }
}
