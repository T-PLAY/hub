...
TEST_CASE( "Measures test" ) {
    ...
    hub::Dof6 dof6( measure );

    hub::Dof6 dof62( 0, 0, 0, 1, 0, 0, 0 );
    hub::Dof6 dof63( 1, 1, 1, 1, 0, 0, 0 );
    auto dof623 = hub::Dof6::slerp( dof62, dof63, 0.5 );
    CHECK( dof623 == hub::Dof6( 0.5, 0.5, 0.5, 1, 0, 0, 0 ) );

    auto dof6Clone = dof6.clone();
    CHECK( dof6 == dof6Clone );

    hub::Dof6 dof64( 1, 2, 3, -1, 0, 0, 0 );
    //    auto dof624 = hub::Dof6::slerp(dof62, dof64, 1.0);
    CHECK( hub::Dof6::slerp( dof62, dof64, 0.0 ) == dof62 );
    //    CHECK(hub::Dof6::slerp(dof62, dof64, 1.0)  == dof64);
    //    CHECK(dof624 == dof64);

    hub::Measure& measure2 = dof62;
    hub::Measure& measure3 = dof63;
    ...
    float matData[16] = { 0 };
    hub::Mat4 mat4( matData );
    CHECK( mat4 != measure2 );
    //    CHECK(measure23 == dof623);
    std::cout << mat4 << std::endl;

    hub::Measure measure4 = std::move( measure );

    CHECK( measure4 != measure2 );
    std::cout << measure4 << std::endl;

    hub::UserData userData( "userData", 0 );
    std::cout << userData << std::endl;

    hub::Measure& measure5 = userData;
    hub::UserData userData2( measure5 );
