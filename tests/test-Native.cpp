...
TEST_CASE( "Native test" ) {
    ...
    const hub::Acquisition ref_acq = std::move(hub::Acquisition(0, 1) << hub::data::Measure(ref_data, 3, ref_resolution));
    ...
    auto onNewStreamer = [](const char* streamName, const hub::SensorSpec* sensorSpec) {
        ...
        for (int iResolution = 0; iResolution < resolutionsSize; ++iResolution) {
            ...
            const int resolutionSize = hub::native::sensorSpec_getResolutionSize(sensorSpec, iResolution);
            CHECK(resolutionSize == hub::res::computeAcquisitionSize(ref_resolution));

            const int dimensionsSize = hub::native::sensorSpec_getDimensionsSize(sensorSpec, iResolution);
            std::cout << "[Example][Viewer] resolutions[" << iResolution
                      << "] dimensions size : " << dimensionsSize << std::endl;
            CHECK(hub::res::computeAcquisitionSize(ref_resolution) == hub::res::computeAcquisitionSize(ref_format, ref_dims));
            CHECK(dimensionsSize == ref_dims.size());

            for (int iDimension = 0; iDimension < dimensionsSize; ++iDimension) {
                const auto dim = hub::native::sensorSpec_getDimension(sensorSpec, iResolution, iDimension);
        ...
        const hub::SensorSpec::MetaData* metaData = hub::native::sensorSpec_getMetaData(sensorSpec);
        CHECK(hub::native::metaData_exists(metaData, "a"));
        CHECK(hub::native::metaData_getInt(metaData, "a") == ref_metaData.at("a").getInt());
        CHECK(hub::native::metaData_getDouble(metaData, "c") == ref_metaData.at("c").getDouble());
...
//                hub::io::OutputStream(ref_streamName, hub::net::ClientSocket(ipv4, port)));
            ...
            if (false) {
                ...
                auto* inputSensor = hub::native::createInputSensor(ref_streamName.c_str(), ipv4.c_str(), port);
                ...
                *outputSensor << ref_acq;
                ...
                auto* inputSensor2 = hub::native::createInputSensor("nonConnected", ipv4.c_str(), port);
