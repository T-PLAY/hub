...
//    CHECK(acq2.getMeasures())
    ...
    CHECK (acq2 == acq3);
    CHECK (acq2 == hub::Acquisition(0, 1) << hub::Measure(data, 3, resolution));

    unsigned char data2[3] { 0, 1, 3};
    CHECK (acq2 != hub::Acquisition(0, 1) << hub::Measure(data2, 3, resolution));

    CHECK ( ! acq2.isInterpolable() );

//    hub::Acquisition acq4 = std::move(hub::Acquisition(0, 0) << hub::Measure(data2, 3, resolution));
//    CHECK ( ! acq4.isInterpolable());
...
//    hub::s_format2isInterpolable
    for (int i = 1; i < (int)hub::Format::COUNT; ++i) {
        ...
        hub::Acquisition acq = std::move(hub::Acquisition(0, 0) << hub::Measure(data, hub::computeAcquisitionSize(resolution), resolution));
...
//    hub::Dof6 dof6(0, 0, 0, 1, 0, 0, 0);
    ...
    hub::Acquisition acq4 = std::move(hub::Acquisition(0, 0) << hub::Dof6(0, 0, 0, 1, 0, 0, 0));
    hub::Acquisition acq5 = std::move(hub::Acquisition(2, 2) << hub::Dof6(1, 1, 1, 1, 0, 0, 0));
    ...
    CHECK (acq45 == (hub::Acquisition(1, 1) << hub::Dof6(0.5, 0.5, 0.5, 1, 0, 0, 0)));
