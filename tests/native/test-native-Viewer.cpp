...
TEST_CASE( "Native Viewer" ) {
    ...
    static const hub::io::Header header_ref { sizeof(int), {'g', 'o', 't'} };

    {
        ...
        auto onNewStream = []( const char* streamName,
        ...
        auto onLogMessage = []( const char* logMessage ) {
            std::cout << "[test-client-NativeViewer] onLogMessage '" << logMessage << "'" << std::endl;
        };

        auto * viewerHandler = hub::native::createViewerHandler(onServerNotFound, onServerConnected, onServerDisconnected, onNewStream, onNewData, onDelStream, onSetProperty, onLogMessage);
