...
class Format
{
  public:
    ...
    //    Format(int size, std::string typeName);
    //    constexpr Format(int size, std::string_view typeName) :
    //        m_size(size)
    ////        m_typeName(typeName)
    //    {
    //    }
    ...
    constexpr std::string typeName() const { return m_typeName; }
    constexpr bool operator==( const Format& format ) const {
        return m_size == format.m_size && !std::memcmp( m_typeName, format.m_typeName, 8 ) &&
               m_isInterpolable == format.m_isInterpolable;
    }

    int m_size          = 0;
    char m_typeName[16] = { 0 };
    bool m_isInterpolable;
    //    char m_description[80] = {0};
};
