...
class any final
{
  public:
    ...
    /// This is because an `any` may be copy constructed into another `any` at any time, so a copy
    ...
    /// Has the same effect as any(std::forward<ValueType>(value)).swap(*this). No effect if a
    /// exception is thrown.
    ///
    /// T shall satisfy the CopyConstructible requirements, otherwise the program is ill-formed.
    /// This is because an `any` may be copy constructed into another `any` at any time, so a copy
...
#        endif
    ...
    void swap( any& rhs ) noexcept {
        if ( this->vtable != rhs.vtable ) {
            ...
            if ( this->vtable != nullptr ) { this->vtable->move( this->storage, rhs.storage ); }
...
#        endif
        ...
        /// The state of the union after this call is unspecified, caller must ensure not to use src
        /// anymore.
        ...
        /// The state of src after this call is unspecified, caller must ensure not to use src
        /// anymore.
...
/// Performs *any_cast<add_const_t<remove_reference_t<ValueType>>>(&operand), or throws bad_any_cast
/// on failure.
template <typename ValueType>
