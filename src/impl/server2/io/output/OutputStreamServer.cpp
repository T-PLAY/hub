...
void OutputStreamServer::initStream() {
    ...
    hub::io::StreamInterface::ServerMessage mess;
    m_data->m_serverSocket->read( mess );
    if ( mess == hub::io::StreamInterface::ServerMessage::FOUND ) {
    ...
    assert( mess == hub::io::StreamInterface::ServerMessage::NOT_FOUND );
    ...
    m_data->m_serverSocket->read( mess );
    assert( mess == hub::io::StreamInterface::ServerMessage::STREAMER_INITED );
    ...
    m_data->m_serverThread = std::make_unique<std::thread>( [this, data]() {
        ...
        while ( !data->m_shutdown ) {
            try {
                ...
                if ( mess == hub::io::StreamInterface::ServerMessage::SERVER_CLOSED ) {
#ifdef DEBUG_OUTPUT_STREAM
                    std::cout << "[OutputStream] server closed" << std::endl;
#endif
                    data->m_serverSocket->write(
                        hub::io::StreamInterface::ClientMessage::CLIENT_SERVER_DOWN );
                    data->m_serverSocket->close();
                    data->m_streamConnected = false;
                    std::this_thread::sleep_for( std::chrono::milliseconds( 1000 ) );
                    continue;
                }
                else if ( mess == hub::io::StreamInterface::ServerMessage::STREAMER_CLOSED ) {
#ifdef DEBUG_OUTPUT_STREAM
                ...
                else if ( mess == hub::io::StreamInterface::ServerMessage::STREAM_VIEWER_INITED ) {
#ifdef DEBUG_OUTPUT_STREAM
        ...
        if ( m_data->m_serverThread != nullptr ) {
            ...
            if ( m_data->m_serverSocket != nullptr && m_data->m_serverSocket->isConnected() )
                m_data->m_serverSocket->write(
                    hub::io::StreamInterface::ClientMessage::STREAMER_CLIENT_CLOSED );
...
void OutputStreamServer::startStreaming() {
    ...
    m_data->m_streamThread = std::make_unique<std::thread>( [data]() {
        ...
        while ( !data->m_killed ) {
            ...
            hub::io::StreamInterface::ClientType clientType;
            streamSock.read( clientType );

            if ( clientType == hub::io::StreamInterface::ClientType::STREAM_VIEWER ) {
            ...
            else if ( clientType == hub::io::StreamInterface::ClientType::KILLER ) {
                if ( data->m_killed ) {}
...
void output::OutputStreamServer::write( const Data_t* data, size_t size ) {
