...
class Header
{
  public:
    ...
    enum class Format {
        ...
        YUYV,        /**< 			32-bit y0, u, y1, v data for every two pixels. Similar to YUV422 but
                        packed in a different order - https://en.wikipedia.org/wiki/YUV */
        ...
        UYVY,        /**< 			Similar to the standard YUYV pixel format, but packed in a different
                        order */
        ...
        DISPARITY32,   /**<	32-bit float-point disparity values. Depth->Disparity conversion :
                          Disparity = Baseline*FocalLength/Depth */
        DOF6,     /**< 			Pose data packed as floats array, containing translation vector (x,     y,
                     z), rotation quaternion (x, y, z, w) */
    ...
    SRC_API Header( const std::string& sensorName = "",
        ...
        mSensorName( sensorName ), mFormat( format ), mDims( dims ), mMetaData( metaData ) {
