...
for mode in "Debug"; do
	for arch in "x64"; do
		...
		mkdir -p $buildDir
		cmake -B $buildDir . -A $arch -DCMAKE_INSTALL_PREFIX=$installDir
	...
	for arch in "x64" "ARM64"; do
			...
			cmake -B $buildDir . -A $arch -DCMAKE_SYSTEM_NAME=WindowsStore -DCMAKE_SYSTEM_VERSION=10.0 -DCMAKE_BUILD_TYPE=$mode -DCMAKE_INSTALL_PREFIX=$installDir
			cmake --build $buildDir --config $mode --parallel
			cmake --install $buildDir --config $mode
	done
