...
//                    res.clear();
//                    res.insert(res.begin(), pixelData, pixelData + image.getHeight() *
//                    image.getWidth());
//                }
//            } else {
//                std::cerr << "Error: cannot load DICOM image (" <<
//                DicomImage::getString(image.getStatus()) << ")" << std::endl; exit(-1);
//            }
//        }
...
unsigned char* readDicomVolume( const char* filename,
    ...
    //            std::copy(std::filesystem::directory_iterator(filepath),
    ...
    *nBytesPerVoxel      = 2;
    ...
    for ( int i = 0; i < *nImages; ++i ) {
        ...
        //                std::cout << "[DicomUtils] loaded image " << i << " '" << filename <<
        //                std::endl;
    }
