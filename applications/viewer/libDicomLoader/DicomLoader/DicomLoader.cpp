...
namespace IO {
    ...
    {
        return (extension.compare(volFileExtension) == 0) || (extension.compare(dicomFileExtension) == 0);
    }
    ...
    Ra::Core::Utils::Color readColor(std::ifstream& input, std::string& name)
    {
        ...
        if (beg == '[' && end == ']') {
            return Ra::Core::Utils::Color(r, g, b);
        }
    ...
    Ra::Core::Asset::FileData* DicomLoader::loadVolFile(const std::string& filename)
    {
        ...
        if (input.is_open()) {
            ...
            if (!checkExpected("sigma_a", attribname)) {
                return nullptr;
            }
            auto sigma_s = readColor(input, attribname);
            if (!checkExpected("sigma_s", attribname)) {
                return nullptr;
            }
            ...
            if (!checkExpected("size", attribname)) {
                return nullptr;
            }

            input >> attribname >> beg;
            if (!checkExpected("density", attribname)) {
                return nullptr;
            }
            if (beg != '[') {
    ...
    Ra::Core::Asset::FileData* DicomLoader::loadDicomFile(const std::string& filename)
    {
        ...
        if (volumeData) {
            ...
            Ra::Core::Vector3 binSize { Scalar(pixelSpacingWidth), Scalar(pixelSpacingHeight), Scalar(sliceThickness) };
            ...
            default:
                LOG(logERROR) << "DicomLoader : unsupported number of componenets : "
                              << bytePerVoxel;
            }
            //        free( volumeData );
            delete[] volumeData;
    ...
    Ra::Core::Asset::FileData* DicomLoader::loadFile(const std::string& filename)
    {
        ...
        if (extension.compare(volFileExtension) == 0) {
            return loadVolFile(filename);
        } else if (extension.compare(dicomFileExtension) == 0) {
            return loadDicomFile(filename);
        }
