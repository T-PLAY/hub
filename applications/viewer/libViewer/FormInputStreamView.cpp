...
InputStreamThread::InputStreamThread( std::unique_ptr<InputStream> inputStream,
                                      ...
                                      QObject* parent ) :
    mInputStream( std::move( inputStream ) ), mSourceType( sourceType ) {}
...
CounterFpsThread::CounterFpsThread( QLabel& label ) : m_labelFps( label ) {}
...
FormInputStreamView::FormInputStreamView( const std::string& sensorName, QWidget* parent ) :
    QWidget( parent ), ui( new Ui::FormInputStreamView ), m_sensorName( sensorName ) {
    ui->setupUi( this );
...
void FormInputStreamView::onStreamingStopped( const std::string& sourceType ) {
    ...
    if ( inputStreamThreads.empty() ) {
        ...
        if ( stringList.empty() ) emit init( m_sensorName );

        if ( m_sourceType2inputStreamThreads.empty() ) emit isEmpty( m_sensorName );
    }
}
