...
InputStreamThread::InputStreamThread(std::unique_ptr<InputStream> inputStream, const std::string& sourceType, QObject* parent)
...
FormInputStreamView::FormInputStreamView(const std::string& sensorName, QWidget* parent)
    : QWidget(parent)
...
const Stream::Acquisition& FormInputStreamView::getAcquisition(const std::string& sourceType) const
{
...
const InputStreamThread& FormInputStreamView::getIputStreamThread(const std::string& sourceType) const
{
...
void FormInputStreamView::onNewAcquisition(const std::string& sourceType)
{
    ...
    if (activeSourceType == sourceType.c_str()) {
        //        std::cout << "[FormInputStreamView] onNewAcquisition('" << sourceType << "')" << std::endl;
...
void FormInputStreamView::onStreamingStopped(const std::string& sourceType)
{
    ...
    //    m_sourceType2inputStreamThreads[sourceType].push_back(std::make_unique<InputStreamThread>(std::make_unique<InputStream>(std::move(iostream)), sourceType));
    auto* inputStreamThread = inputStreamThreads.front().get();
    QObject::disconnect(inputStreamThread, &InputStreamThread::newAcquisition, this, &FormInputStreamView::onNewAcquisition);
    QObject::disconnect(inputStreamThread, &InputStreamThread::streamingStopped, this, &FormInputStreamView::onStreamingStopped);
    ...
    if (inputStreamThreads.empty()) {
        std::cout << "[FormInputStreamView] onStreamingStopped(" << sourceType << ") no stream left" << std::endl;
