...
public:
	...
	// Connection Methods
	/////////////////////////////////////////////////////////////////////////////////
	ePiCommType	DiscoverCnx	( BOOL bInitCfg=TRUE, LPCTSTR szSrcCalPath=0 );	// Legacy connection method
	...
	VOID	SetSrcCalFilePath	( LPCTSTR szSrcCalPath );	// Source Calibration file needed for G4 initialization
	VOID	SetCmdPosUnits		( ePDIposUnits e );			// Position units to be used in all subsequent tracker commands
	VOID	SetCmdOriUnits		( ePDIoriUnits e );			// Orientation units to be used in all subsequent commands
	BOOL	SetPNODataList		( const CPDImdat & );		// Set/Get a CPDImdat configuration for legacy P&O output
	...
	BOOL	SetFrameOfRef	( const PDI7vec & );	// Translation & Rotation defined together (Rotation units as defined by SetCmdOriUnits()
	...
	BOOL	SetFrameOfRefTRANS( const PDIpos & );	// Tranlation only
													// Get
	BOOL	GetFrameOfRef	( PDI7vec & );			// Transation & Rotation read together (Rotation units as defined by SetCmdOriUnits()
	...
	/////////////////////////////////////////////////////////////////////////////////
	// Increment 
	BOOL	SetSIncrement ( const INT nHub, const INT nSens, const FLOAT fPosIncr, const FLOAT fOriIncr, BOOL bMap=FALSE );
	...
	BOOL	SetSourceFrame	( const PDIori & );		// Sets system Frame of Reference Rotation in Euler Degrees
	BOOL	GetSourceFrame	( PDIori & );			// Sets system Frame of Reference Rotation in Euler Degrees

	BOOL	SetSDataList	( INT nStation, const CPDImdat & d ); // Sets motion data output list on all hubs/all sensors
	BOOL	GetSDataList	( INT nStation, CPDImdat & d );		  // Gets motion data output list from hub 0/sensor 0

	BOOL	SetSBoresight	( INT nStation, const PDIori &, BOOL bReset=FALSE );	// Set boresight on all hubs/nStation in Euler Degrees, no RESET
	BOOL	GetSBoresight	( INT nStation, PDIori & );			// Get boresight from hub0, nStation in Euler Degrees
	...
	// P&O Collection methods - Native G4 hierarchical format		// Data is organized in blocks of type G4_HUBDATA
	BOOL	ReadSinglePnoG4	  ( HWND hwnd );		 
	BOOL	ReadSinglePnoBufG4( PBYTE & pBuf, DWORD & dwSize );
	BOOL	StartContPnoG4	  ( HWND hwnd );
	BOOL	StopContPnoG4	  ( VOID )
							  { return StopContPno(); }
	
	// Other G4 Run-Time commands
	BOOL	GetActiveHubs	( INT & nHubCount, INT HubIDs[G4_MAX_HUB_COUNT] );	// Read list of currently active hub ids managed by the system.
	BOOL	IsHubActive		( const INT & nHub ); // Returns TRUE if hub is currently Active.  Does not refresh Active Hub List
	BOOL	GetHubSensorMap( const INT & nHub, DWORD & dwMap ); // Read current station map from specified hub
	...
	BOOL	GetSourceMap	( INT & nSrcCount, PDI7vec SrcMap[] );	// Read list of sources and their position & rotation
...
inline BOOL	CPDIg4::GetSDataList	( INT nStation, CPDImdat & d )  // Gets motion data output list from hub 0/sensor 0
{
	GetPNODataList( d );
	return TRUE;
}
