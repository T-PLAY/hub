...
public:
	...
	virtual ePiCommType	DiscoverCnx(BOOL bInitCfg = TRUE);			// Search for USB, then Serial Connection to device
							//{ return CPDIdev::DiscoverCnx( bInitCfg ); }
	BOOL		ConnectSer  ( BOOL bInitCfg=FALSE )						// Attempts to connect via current serial port parameters
	...
	BOOL		SetSerialIF	( CPDIser * pS ) // Sets current serial port parameters locally: if no connection, does not configure device
							{ return CPDIdev::SetSerialIF( pS ); }
	BOOL		GetSerialIF	( CPDIser * pS ) // Returns current serial port parameters locally: if no connection, does not configure device
	...
	BOOL	SetPresetFilters( const ePDIFilter & eF );	// Sets Position AND Attitude Filters to OFF, LOW, MED, HEAVY. Do not use CUSTOM!
	...
	virtual BOOL	ClearSBoresight	( INT nStation )							// UN-Boresight station ('b' command )
			...
			BOOL	GetSDataList	( INT nStation, CPDImdat & mdat  )			// Gets motion data output list per station ( 'O' Command )
	...
	virtual BOOL	GetSBoresight	( INT nStation, PDIori & ori )				// Get boresight angles per station ( 'G' Command )
									{ return CPDIdev::GetSBoresight( nStation, ori ); }


			BOOL	GetSHemisphere	( INT nStation, PDI3vec & vec )				// Get Hemisphere Of Operation per station ( 'H' Command )
									{ return CPDIdev::GetSHemisphere( nStation, vec ); }
			BOOL	GetSHemiTrack	( INT nStation, BOOL & bHTrack )			// Query Hemisphere Tracking per station ('H' Command )
									{ return CPDIdev::GetSHemiTrack( nStation, bHTrack ); }
			BOOL	GetSStylusMode	( INT nStation, ePiStylusMode & eMode )		// n/a FT-I&II Get Stylus Mode per station ( 'e' Command )
			...
			BOOL	GetSSourceFrame	( INT nStation, PDIori & );			// Gets Source Frame Rotation "Transmitter Mounting Frame" ( 'r' Command )

			BOOL	GetSTipOffset	( INT nStation, PDIpos & );			// Get Station Tip Offset ('N' Command )

			BOOL	SetSDataList	( INT nStation, const CPDImdat & mdat  )		// Sets motion data output list per station ( 'O' Command )
	...
	virtual BOOL	SetSBoresight	( INT nStation, const PDIori & ori )	// Set boresight per station ( 'G' and 'B' Commands )
			...
			BOOL	SetSHemisphere	( INT nStation, const PDI3vec & vec )			// Set Hemisphere Of Operation per station ( 'H' Command )
			...
			BOOL	SetSStylusMode	( INT nStation, ePiStylusMode eMode )			// Set Stylus Mode per station ( 'e' Command )
			...
			BOOL	SetSSourceFrame	( INT nStation, const PDIori & );		// Sets Source Frame Rotation "Transmitter Mounting Frame" ( 'r' Command )
	...
	VOID	EmulateLibertyPno( BOOL bEm );		// Enable/disable Liberty-style P&O frame headers and frame bundling
	...
	BOOL	WhoAmI			( LPCSTR & szBuf )					// Issues tracker System Status command ('S' command ); puts result in argument 
							{ return CPDIdev::WhoAmI( szBuf ); }
	//n/a Fastrak BOOL	WhoAmISensor	( INT nSensor,  LPCSTR & szBuf )	// Issues sensor WhoAmI command ( '^V' command ); puts result in argument 
	//						{ return CPDIdev::WhoAmISensor( nSensor, szBuf ); }

	// The following are better suited for Unicode builds
	BOOL	TxtCmdW			( LPCWSTR szWCmd, DWORD & dwRspWSize, LPCWSTR szWRsp=0 )	// Sends generic text command to device
							{ return CPDIdev::TxtCmd( szWCmd, dwRspWSize, szWRsp ); }
	BOOL	WhoAmI			( LPCTSTR szTBuf, DWORD nBufSize )
							{ return CPDIdev::WhoAmI( szTBuf, nBufSize ); }
	...
	BOOL    StartSocketExport  ( LPCTSTR szInterfaceAddress=PDI_EXPORT_TCP_ADDR,
	                           ...
	                           { return CPDIdev::StartSocketExport(szInterfaceAddress, nSocketPort); }//Starts raw P&O on addr:port
	BOOL    CancelSocketExport ( VOID )
	                           { return CPDIdev::CancelSocketExport(); } // Cancels raw P&O export on addr:port
