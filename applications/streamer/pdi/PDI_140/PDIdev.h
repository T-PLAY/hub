...
public:
	...
	virtual BOOL	GetSDataList	( INT nStation, CPDImdat &   );		// Gets motion data output list per station ( 'O' Command )
			BOOL	GetSAlignment	( INT nStation, PDI3vec & O, 
													PDI3vec & X, 
													PDI3vec & Y );		// Gets Alignment Reference Frame per station ( 'A' Command )
	virtual BOOL	GetSBoresight	( INT nStation, PDIori & );			// Get boresight per station ( 'B' Command )
	...
	virtual BOOL	SetSDataList	( INT nStation, const CPDImdat &   );			// Sets motion data output list per station ( 'O' Command )
			BOOL	SetSAlignment	( INT nStation, const PDI3vec & O, 
													const PDI3vec & X, 
													const PDI3vec & Y );			// Sets Alignment Reference Frame per station ( 'A' Command )
	virtual BOOL	SetSBoresight	( INT nStation, const PDIori &, BOOL bReset=FALSE );	// Set boresight per station ( 'B' Command )
	...
	// The following are better suited for Unicode builds
	BOOL	TxtCmd			( LPCWSTR szWCmd, DWORD & dwRspWSize, LPCWSTR szWRsp=0 );	// Sends generic text command to device
	...
	VOID	Log			( BOOL, INT nWriteFlushCount=0 );	// Enables tracker I/O logging to file PICMDIF.LOG
															// nWriteFlushCount=zero, only flush when logfile is closed.
	VOID	LogASCII	( BOOL );		// Debug Trace I/O output and Log file output in ASCII, rather than hex
	...
	// UDP Export Methods
	////////////////////////////////////
	BOOL	StartUDPExport  (ULONG in_addr, SHORT nPort = PDI_EXPORT_UDP_PORT); //Starts raw P&O on UDP port using host-byte order inet address
