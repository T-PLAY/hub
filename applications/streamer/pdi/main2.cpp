...
/*
Polhemus Tracker Terminal version 1.0.3 -- Terminal Interface to Polhemus Trackers: Fastrak,
Patriot, and Liberty Copyright  ©  2009-2011  Polhemus, Inc.
...
int main( int argc, char* argv[] ) {
    ...
    READ_WRITE_STRUCT writeStruct = {
        &pong, keepLooping, &thread_id, NULL }; // will add textview after it's created
    ...
    g_object_set_data(
        G_OBJECT( connect ), "disconnect", (gpointer)disconnect ); // to enable disconnect button
    g_object_set_data(
        G_OBJECT( connect ), "readstruct", (gpointer)&readStruct ); // to start read thread
    g_object_set_data(
        G_OBJECT( connect ), "writestruct", (gpointer)&writeStruct ); // to start timout write fxn
    g_object_set_data(
        G_OBJECT( disconnect ), "connect", (gpointer)connect ); // to enable connect button
    g_object_set_data( G_OBJECT( disconnect ),
                       "readstruct",
                       (gpointer)&readStruct ); // to kill read thread and stop timeout fxn
    g_signal_connect(
        G_OBJECT( connect ), "clicked", G_CALLBACK( OnConnect ), (gpointer)&cnxStruct );
    g_signal_connect(
        G_OBJECT( disconnect ), "clicked", G_CALLBACK( OnDisconnect ), (gpointer)&cnxStruct );
    ...
    cnxStruct.portChooser =
        gtk_file_chooser_button_new( "Choose the RS232 Port", GTK_FILE_CHOOSER_ACTION_OPEN );
    ...
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( cnxStruct.cnxType == USB_CNX ? usb : rs232 ),
                                  TRUE );
    ...
    g_signal_connect(
        G_OBJECT( cs.browse ), "clicked", G_CALLBACK( OnCaptureBrowse ), (gpointer)&cs );
    ...
    gtk_scrolled_window_set_policy(
        GTK_SCROLLED_WINDOW( scrWin ), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC );
    ...
    cmd = gtk_entry_new();
    g_object_set_data( G_OBJECT( connect ),
                       "entry",
                       (gpointer)cmd ); // so that we can set the cursor here on connect
    ...
    gtk_grid_attach_next_to(
        GTK_GRID( grd5 ), GTK_WIDGET( cnxStruct.cnxLabel ), NULL, GTK_POS_LEFT, 1, 1 );
    ...
    g_signal_connect(
        G_OBJECT( cmd ), "key-press-event", G_CALLBACK( CommandEntered ), &cnxStruct );
...
void OnAbout( GtkWidget* w, GtkWindow* win ) {
    ...
    gtk_show_about_dialog( win,
                           ...
                           author,
                           "comments",
                           "Terminal Interface for Polhemus Trackers",
                           "copyright",
                           copyright,
                           "version",
                           PACKAGE_VERSION,
                           "program-name",
                           "Polhemus Tracker Terminal",
                           "website",
...
int Browse4CaptureFile( LPCAP_STRUCT pcs ) {
    ...
    dlg = gtk_file_chooser_dialog_new( "Specify File to write Captured Data",
                                       NULL,
...
int OpenCaptureFile( LPCAP_STRUCT pcs ) {
    ...
    if ( pcs->fCap == NULL ) {
        dlg = gtk_message_dialog_new( GTK_WINDOW( pcs->win ),
                                      GTK_DIALOG_MODAL,
                                      GTK_MESSAGE_WARNING,
                                      GTK_BUTTONS_OK,
                                      "Unable to open file %s for writing.  Permissions?",
                                      pcs->filename );
    ...
    if ( pcs->cnxType == USB_CNX ) {
        do {
            cnxSuccess = pcs->pTrak->UsbConnect( usbTrkParams[pcs->trackerType].vid,
                                                 ...
                                                 usbTrkParams[pcs->trackerType].writeEp,
                                                 usbTrkParams[pcs->trackerType].readEp );
        ...
        if ( cnxSuccess != 0 ) {
            msg = gtk_message_dialog_new( NULL,
                                          ...
                                          GTK_MESSAGE_WARNING,
                                          GTK_BUTTONS_OK,
                                          "Unable to connect to RS232 port %s.",
                                          port );
...
int QueryUser4TrackerType( int& trk ) {
    ...
    dlg = gtk_dialog_new_with_buttons( "Select Tracker Type",
                                       NULL,
                                       GTK_DIALOG_MODAL,
                                       GTK_STOCK_OK,
                                       GTK_RESPONSE_ACCEPT,
    ...
    g_signal_connect(
        G_OBJECT( rb0 ), "toggled", G_CALLBACK( TrackerSelect ), (gpointer)&trk_type );

    for ( long i = 1; i < NUM_SUPP_TRKS; i++ ) {
        rb =
            gtk_radio_button_new_with_label_from_widget( GTK_RADIO_BUTTON( rb0 ), trackerNames[i] );
        gtk_box_pack_start( GTK_BOX( content ), rb, TRUE, TRUE, 5 );
        g_object_set_data( G_OBJECT( rb ), "index", (gpointer)i );
        g_signal_connect(
            G_OBJECT( rb ), "toggled", G_CALLBACK( TrackerSelect ), (gpointer)&trk_type );
    }
...
gboolean CommandEntered( GtkEntry* w, GdkEventKey* key, LPCNX_STRUCT pcs ) {
    ...
    int validKey = ( keyval == GDK_KEY_asterisk ) || ( keyval == GDK_KEY_comma ) ||
                   ( keyval == GDK_KEY_period ) || ( keyval == GDK_KEY_at );

    if ( ( key->type == GDK_KEY_PRESS ) &&
         ( ( g_ascii_isalnum( keyval ) ) || ( keyval == GDK_KEY_Return ) || validKey ) ) {
        if ( key->state & GDK_CONTROL_MASK ) { // a ctrl key
            ...
            g_free( txt );
            rv = TRUE; // ^V causes the clipboard to be pasted to the entry. return TRUE to prevent
                       // it.
        }
...
void* ReadTrackerThread( void* pParam ) {
    ...
    do {
        pTrak->WriteTrkData(
            (void*)"\r", 1 ); // send just a cr, should return an short "Invalid Command" response
        usleep( 100000 );
