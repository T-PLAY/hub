...
//			{0x0f44, 0xef12, 0x02, 0x82}};  	// Patriot
	...
	public:
		TrackerVersionInfo(WhoAmIBinaryResponse whoAmIBinaryResponse) {
			...
			case PATRIOT:
				if (trackerName.find("Patriot") == trackerName.npos)
					throw new exception();
				cout << "\nConnected to Patriot" << endl;
				isPatriotBool = true;
				break;
			case LIBERTY:
				if (trackerName.find("Liberty") == trackerName.npos)
					throw new exception();
		...
		bool isLiberty() {
			return isLibertyBool;
		}

		bool isPatriot() {
			return isPatriotBool;
		}

        bool getSensorCount() const {
			return sensorCount;
		}
	};
	...
	public:
		 ...
		 */
		void enableHemisphereTracking(
				const Sensor& sensor, const Position& origin);
