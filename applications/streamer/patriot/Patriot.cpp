...
bool Patriot::connect() {
    ...
    handle = libusb_open_device_with_vid_pid(
        0, USB_PARMS[PATRIOT].vendorID, USB_PARMS[PATRIOT].productID );
...
void Patriot::updateSensorActiveState( signed int activeStationBitmap ) {
    for ( unsigned int bitmapPosition = 0; bitmapPosition < PATRIOT_SENSOR_COUNT; bitmapPosition++ )
        sensors[bitmapPosition]->setActive( activeStationBitmap & 1 << bitmapPosition );
}
...
bool Patriot::checkResponseData( BinaryResponseHeader& binaryResponsebuffer ) {
    if ( binaryResponsebuffer.errorIndicator != ' ' &&
        ...
        cout << "Response error: " << errorDescriptions[binaryResponsebuffer.errorIndicator]
             << "\n";
        return false;
    }
    ...
    if ( binaryResponsebuffer.frameTag1 != 'P' || binaryResponsebuffer.frameTag2 != 'A' )
        return false;
    else
        return true;
}
...
bool Patriot::receive( SingleDataBinaryResponse2_4_1& binaryResponse ) {
    receive( (void*)&binaryResponse, sizeof( SingleDataBinaryResponse2_4_1 ) );
...
bool Patriot::receive( ActiveStationStateBinaryResponse& binaryResponse ) {
    receive( (void*)&binaryResponse, sizeof( ActiveStationStateBinaryResponse ) );
...
bool Patriot::receive( void* binaryResponsebuffer, int bufferSize ) {
    ...
    while ( downloadSize != bufferSize ) {
        ...
        int ret = libusb_bulk_transfer( handle,
                                        USB_PARMS[PATRIOT].bulkReadEndPoint,
...
void Patriot::send( string data ) const {
    ...
    if ( uploadSize == 0 ) throw new exception();
}
...
BinaryPosition Patriot::getMeasurement() {
    ...
    if ( !getIsInBinaryOutputMode() ) putTrackerIntoBinaryMode();
...
bool Patriot::isConnected() const {
    return connected;
}

const vector<Sensor*>& Patriot::getSensors() const {
    return sensors;
}
...
void Patriot::setAlignmentReferenceOrigin( char sensorID, Position& origin ) {
    ...
    alignmentCommand << origin.getXpos() << ',' << origin.getYpos() << ',' << origin.getZpos();
 ...
 * See TAP Cmd IF Spec.
 */
void Patriot::putTrackerIntoASCIIMode() {
    setTrackerBinaryMode( false );
}
 ...
 * See TAP Cmd IF Spec.
 */
void Patriot::putTrackerIntoBinaryMode() {
    setTrackerBinaryMode( true );
}
...
bool Patriot::isTrackerInMetricUnits() {
    ...
    if ( unitsBinaryResponse.inchesOrCentimeters == CENTIMETERS ) return true;
    if ( unitsBinaryResponse.inchesOrCentimeters == INCHES ) return false;
    throw new exception();
}
 ...
 */
void Patriot::enableHemisphereTracking( const Sensor& sensor, const Position& origin ) {
