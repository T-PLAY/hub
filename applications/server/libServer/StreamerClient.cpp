    ...
    : Client(server, iClient)
{
    ...
    try {
        m_inputSensor = std::make_unique<hub::InputSensor>(StreamViewerInterface(std::move(sock)));
            ...
            while (true) {
                ...
                while (it2 != m_syncViewers.end()) {
                    ...
                    do {
                        ...
                        assert(syncAcqs.begin()->m_start != std::next(syncAcqs.begin())->m_start);
                    ...
                    const auto closestAcq = (std::abs(itLeftAcq->m_start - masterAcq.m_start) > std::abs(itRightAcq->m_start - masterAcq.m_start))
                        ? (itRightAcq)
                        : (itLeftAcq);
...
void StreamerClient::addStreamViewer(StreamViewerClient* streamViewer)
{
    ...
    if (syncStreamName == "") {
        m_mtxStreamViewers.lock();
        assert(std::find(m_streamViewers.begin(), m_streamViewers.end(), streamViewer) == m_streamViewers.end());
        ...
        assert(std::find(syncViewers.begin(), syncViewers.end(), streamViewer) == syncViewers.end());
