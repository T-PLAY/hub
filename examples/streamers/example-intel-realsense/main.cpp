    ...
    while ( true ) {
        try {
            ...
            while ( true ) {
                ...
                depthStream << ( hub::Acquisition { depth.get_frame_metadata(
                                                        RS2_FRAME_METADATA_BACKEND_TIMESTAMP ),
                                                    depth.get_frame_metadata(
                                                        RS2_FRAME_METADATA_TIME_OF_ARRIVAL ) }
                                 << hub::Measure { (unsigned char*)depth.get_data(),
                                                   depthSize,
                ...
                rgbStream << ( hub::Acquisition {
                               ...
                               << hub::Measure { (unsigned char*)color.get_data(),
                                                 colorSize,
                                                 { { 640, 480 }, hub::Format::RGB8 } } );
#endif
                ...
                infraredStream << ( hub::Acquisition { ir.get_frame_metadata(
                                                           RS2_FRAME_METADATA_BACKEND_TIMESTAMP ),
                                                       ir.get_frame_metadata(
                                                           RS2_FRAME_METADATA_TIME_OF_ARRIVAL ) }
                                    << hub::Measure { (unsigned char*)ir.get_data(),
                                                      infraredSize,
